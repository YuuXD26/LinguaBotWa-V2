const sessionName = "YuuBot";
const botName = "Lynx-Bot";
const ownerName = "YuuXD";
const donet = "Coming Soon";
const owner = ["6289689075040"]; // Put your number here ex: ["62xxxxxxxxx"]
const now = new Date();
...
async function startHisoka() {
  ...
  const client = yuuConnect({
    logger: pino({ level: "silent" }),
    printQRInTerminal: true,
    browser: ["LynxBot - YuuXD", "Safari", "5.1.7"],
    auth: state,
  });
  ...
  client.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    try {
      ...
      require("./yuu")(client, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
